  ...
  try
    {
      ...
      if(improv){
        ...
        read(xml_in,"/ThreeptIniParams/Improvement/NuS",photon.nu_s);
        read(xml_in,"/ThreeptIniParams/Improvement/Xi0",photon.xi0);
        read(xml_in,"/ThreeptIniParams/Improvement/a_s",photon.a_s);
        read(xml_in,"/ThreeptIniParams/Improvement/a_t",photon.a_t);        
        read(xml_in,"/ThreeptIniParams/Improvement/InvFac2",photon.coeff);
    ...
    try
      {
        ...
        read(xml_kf_in,"/kfac/numMatrixElem",num_matrix_elem);
        read(xml_kf_in,"/kfac/MatrixName",matrix_name);       
  ...
  */  
  ...
  for(int i = 0; i < num; i++ )
  {
    ...
    for(ADAT::MapObject<Hadron::KeyHadronSUNNPartNPtCorr_t, std::vector<ENSEM::VectorComplex> >::const_iterator  corr = corrs.begin(); corr != corrs.end(); ++corr) 
    {
      ...
      vector<std::stringstream> kfpath(num_matrix_elem);
      vector<EnsemComplex> kfac_ensem(num_matrix_elem);
      double check_zero;
      ...
      for(int matrix_num = 0; matrix_num < num_matrix_elem; matrix_num++){
        kfpath[matrix_num] << path.str() + matrix_name[num];
      ...
      for(int t = 0; t < corr->second[0].numElem(); t++){ //numElem gives the number of observables in the ensem so use any index for convinience use 0
        ...
        for(size_t bin = 0; bin < corr->second.size(); bin++){//corr num issue
          ...
          //if(!divkfac){
          yt_rl.elem(bin) = std::exp(Ef*(dt - t))/(sqrt(CurrFac)) * std::exp(Ei*t) * toScalar(yt_rl.elem(bin)); //multiplying by the exp
          yt_im.elem(bin) = std::exp(Ef*(dt - t))/(sqrt(CurrFac)) * std::exp(Ei*t) * toScalar(yt_im.elem(bin)); 
          ...
          //   complex<double> kf = toScalar(kfac_ensem.elem(bin));
      ...
      {
        ostringstream outfile; outfile << path.str() << "/corr_wo_exp.jack";
