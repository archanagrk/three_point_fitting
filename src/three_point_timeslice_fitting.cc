				    ...
				    )
{
  ...
  /* perform constant fits */
  {
    ...
    t_pairs_in = get_all_t_ranges(control,1,1,1);
    ...
    cout << "*********************************************************************************" << endl;
      ...
      for(int j = 0 ; j < control.tmin_max.size() ; j++){
        ...
        cout << *x_tlow[j] << "cnst__t_low....";
        cout << *x_thigh[j] << "cnst__t_high......" << endl;
  ...
  if(!control.only_cnst)
  {
    // src_exp fits
    {
      ...
      t_pairs_in = get_all_t_ranges(control,0,1,0);
      ...
      cout << "*********************************************************************************" << endl;
        for(int dt_count = 0 ; dt_count < t_pairs[i].size() ; dt_count++)
        {
          ...
          cout << "src_exp__low...." << *x_t_low_dt << endl;
    ...
    // snk_exp_fits
    {
      ...
      t_pairs_in = get_all_t_ranges(control,0,0,1);
      ...
      cout << "*********************************************************************************" << endl;
        ...
        for(int dt_count = 0 ; dt_count < t_pairs[i].size() ; dt_count++)
        {
          ...
          cout << "snk_exp__low...." << *x_t_low_dt << endl;
    ...
    if(!control.only_one_exp)
    {
     ...
     vector<bool> previous( accepted_data.size(), false );
      ...
      t_pairs_in = get_all_t_ranges(control,0,1,1);
      ...
      cart_product(t_pairs, t_pairsTemp, t_pairs_in.begin(), t_pairs_in.end());
        ...
        cout << "*********************************************************************************" << endl;
          ...
          for(int j = 0 ; j < control.tmin_max.size() ; j++){
            ...
            cout << *x_tlow[j] << "two_exp_t__low....";
            cout << *x_thigh[j] << "two_exp_t__high......" << endl;
            active_data = active_data || data_in_x_range( data, make_pair(x_tlow[j],x_thigh[j]) );
          }
  ...
  vector<AvgFit*> fits_resize;
  map<double, AvgFit*, std::greater<double> > ordered_fits = make_ordered_list( fits, fit_qual );

  for(auto it = ordered_fits.begin(); it != ordered_fits.end(); it++){

    fits_resize.push_back(it->second);

    if(fits_resize.size() >= 30) {break;}

  }

  cout << endl << "carrying out ensem fits to the best" << " " << fits_resize.size() << "............." << endl;
